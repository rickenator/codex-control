...
Open Consiglio, select **Mobile**, enter the public HTTPS URL, and choose **Enable & create token**. Consiglio generates a 256-bit token, encrypts it with the operating-system credential store, and displays a one-time QR enrollment code. In the mobile app choose **Scan desktop pairing code**; the scanned endpoint and token are strictly validated before the first authenticated health check. Closing the desktop dialog removes the visible token and QR code. Use **Rotate token** to revoke every client using the previous token, or **Disable & revoke** to stop the bridge and delete the saved encrypted token.
...
- Node.js 22 or newer;
- Android Studio and Android SDK 36 for Android builds (Android 8/API 26 minimum);
...
- QR enrollment accepts only the versioned `consiglio://pair/v1` payload shape, rejects extra fields and oversized input, then applies the same HTTPS and token validation as manual pairing.
